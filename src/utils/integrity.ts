...
/** Synchronous SHA-256 keeps IndexedDB writes atomic while the transaction is active. */
export function sha256Hex(value: string): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 =
        (h! + upperSigma1 + choice + SHA_256_ROUND_CONSTANTS[index]! + words[index]!) >>> 0
...
export function canonicalJson(value: unknown): string {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value))
      throw new Error('Une empreinte ne peut pas contenir de nombre invalide.')
    return JSON.stringify(value)
  }
  ...
  throw new Error(`Type non pris en charge pour l’empreinte : ${typeof value}.`)
}
